...
- When the current pending `branching_logic` question has a small finite answer set, the backend may attach `context.quick_reply` metadata so the frontend can render selectable response buttons. The visible assistant markdown stays intern-friendly; internal workflow and question ids stay in response context and hidden `pbsg-state` comments for backend state recovery. Button labels are friendly, and button clicks are sent back as normal user messages with free-text input still available.
...
The four output formats (A/B/C/D) show `**Selected Stream:**` with a human-readable stream name instead of visible `GEN3-*` ids. Route labels remain visible where they are the actual recommendation, while `evals/pbsg_golden_set_eval.py` reads selected workflow ids from `context.pbsg_triage_state` or hidden `pbsg-state` metadata.
